...
\title{Atomic Physics: Report Assignment Solutions}
...
\section*{Problem 1: Quantization of the Electromagnetic Field and Atom-Field Interactions}

\subsection*{1. Explain why the transition probability for photon absorption is proportional to the photon number $n$.}
...
\subsection*{2. Explain why the transition probability for photon emission is proportional to $n+1$.}
...
\subsection*{3. In the factor $n+1$, explain what the terms proportional to $n$ and to $1$ physically represent.}
The factor $n+1$ can be physically separated into two distinct components:
\begin{itemize}
    \item \textbf{The term proportional to $n$:} This represents \textbf{stimulated emission}. It is an emission process triggered by the presence of existing photons in the mode. The rate of this process increases linearly with the ambient photon density (or light intensity).
    \item \textbf{The term proportional to $1$:} This represents \textbf{spontaneous emission}. This process occurs independently of any external photons. Even if the field is initially in the vacuum state ($n=0$), this term ensures that the emission probability remains non-zero.
\end{itemize}

\subsection*{4. Explain the difference between spontaneous emission and stimulated emission, and discuss why spontaneous emission can occur even in vacuum, relating your answer to the role of the quantized electromagnetic field.}
\begin{itemize}
    ...
    \item \textbf{Occurrence in Vacuum:} Classical electromagnetic theory cannot explain why an isolated atom in a perfect vacuum should ever spontaneously decay. However, in quantum field theory, the electromagnetic field is quantized. The lowest energy state of a quantized field mode is the vacuum state $|0\rangle$, which possesses a non-zero \textbf{vacuum fluctuation} (corresponding to the zero-point energy $\frac{1}{2}\hbar\omega$). These fundamental, stochastic fluctuations of the vacuum field interact with the atomic dipole, perturbing the excited state and driving the transition to the ground state. This is why the spontaneous emission term ($1$ in the $n+1$ factor) remains active even in a vacuum.
\end{itemize}

\subsection*{5. The lifetime of the $2^{3}S_{1}$ state of helium is about 8000 s, whereas typical electric-dipole-allowed transitions have lifetimes of the order of $10^{-8}$ s. Using the expression for the spontaneous emission rate derived in class, explain the physical origin of this enormous difference.}
...
\end{enumerate}
Because the electric dipole transition is completely forbidden ($A_{E1} = 0$), the decay can only proceed via extremely weak, higher-order processes, specifically a relativistic \textbf{magnetic dipole ($M1$) transition}. Since the matrix elements for $M1$ transitions are several orders of magnitude smaller than those for $E1$ transitions, the transition rate $A$ becomes incredibly small, resulting in an exceptionally long lifetime of $\approx 8000\text{ s}$.

\newpage

\section*{Problem 2: Thermal Equilibrium of Atoms in Blackbody Radiation}

\subsection*{1. Explain why the internal-state populations of atoms interacting with blackbody radiation obey the Boltzmann distribution in thermal equilibrium. In your explanation, refer to the balance between absorption, stimulated emission, and spontaneous emission.}
...
\]
This is precisely the \textbf{Boltzmann distribution}. Thus, the detailed balance between absorption, stimulated emission, and spontaneous emission naturally satisfies the thermodynamic requirement under blackbody radiation.

\subsection*{2. Explain how absorption, stimulated emission, and spontaneous emission contribute to (i) the friction force acting on the atom, and (ii) momentum diffusion.}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textit{Absorption:} The moving atom perceives higher-frequency (blue-shifted) photons in its direction of motion. Depending on the spectral gradient at the atomic resonance $\omega_A$, the atom absorbs photons at different rates from the forward and backward directions. This differential absorption transfers a net momentum opposite to the velocity, acting as a decelerating \textbf{friction force}.
        \textit{Stimulated Emission:} Stimulated emission is induced by the directional photons of the radiation field. Because the forward field intensity is modified by the Doppler shift, it stimulates emission preferentially along the axis of motion. Combined with absorption, the net balance determines the magnitude and sign of the directional damping friction.
        \item \textit{Spontaneous Emission:} Spontaneous emission is spatially isotropic on average. Therefore, the net momentum transferred over many random directions averages out to zero, meaning it \textbf{does not contribute directly to the directional friction force}.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textit{Absorption:} Photons are absorbed at discrete, stochastic times from various directions. Even if the radiation is isotropic, these discrete momentum steps ($\pm \hbar\mathbf{k}$) cause the atom's momentum to execute a random walk, contributing to momentum diffusion.
        \item \textit{Spontaneous Emission:} Although the average force from spontaneous emission is zero, each individual decay event emits a photon in a random direction, imparting a discrete recoil momentum $-\hbar\mathbf{k}$ to the atom. This stochastic recoil acts as a major source of \textbf{momentum diffusion}.
        \item \textit{Stimulated Emission:} Stimulated emission events also occur stochastically along the directions of the mode photons. The randomness of when these emission events occur adds further fluctuations to the net momentum, expanding the momentum diffusion tensor.
    \end{itemize}
\end{itemize}

\newpage

\section*{Problem 3: Thermalization in Blackbody Radiation}

\subsection*{1. Calculate the average photon number $\overline{n}(\omega_{A})$ at the resonance frequency.}
...
\]
Thus, the average photon number at the resonance frequency is $\overline{n}(\omega_A) \approx \mathbf{0.0174}$.

\subsection*{2. Using the expression for the friction coefficient, or equivalently the thermalization time, derived in class, estimate the thermalization time of sodium atoms.}
...
\[
\tau_{\text{therm}} = \frac{M}{\alpha} \approx \frac{3.8 \times 10^{-26}\text{ kg}}{5.19 \times 10^{-14}\text{ kg/s}} \approx \mathbf{7.3 \times 10^{-13}\text{ s}}
\]
...
\subsection*{3. Based on your results, explain why blackbody radiation alone cannot efficiently cool atoms.}
